...
// app.use("/api/v1/notification/<module>", moduleRoutes);

async function main() {
  if (process.env.MONGODB_URI) {
    await connectMongo(process.env.MONGODB_URI, "shop_notification");
  }
  const port = Number(process.env.PORT ?? SERVICE_PORTS["notification"]);
