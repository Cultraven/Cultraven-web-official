...
// app.use("/api/v1/delivery/<module>", moduleRoutes);

async function main() {
  if (process.env.MONGODB_URI) {
    await connectMongo(process.env.MONGODB_URI, "shop_delivery");
  }
  const port = Number(process.env.PORT ?? SERVICE_PORTS["delivery"]);
  app.listen(port, () => log.info(`delivery-service listening on :${port}`));
}
