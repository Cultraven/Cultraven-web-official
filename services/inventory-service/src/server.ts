...
// app.use("/api/v1/inventory/<module>", moduleRoutes);

async function main() {
  if (process.env.MONGODB_URI) {
    await connectMongo(process.env.MONGODB_URI, "shop_inventory");
  }
  const port = Number(process.env.PORT ?? SERVICE_PORTS["inventory"]);
